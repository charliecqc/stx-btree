				...
				node_t *find_preds(node_t **preds, node_t **succs, int n, skiplist_t *sl, key_type key, enum unlink unlink) {
					...
					for(int level = sl->high_water - 1; level >= 0; --level) {
						...
						while (item != NULL) {
							...
							cout << "s4 find_preds: key " << STRIP_MARK(item->max) << " val " << item->val << endl;
#endif
							if(EXPECT_TRUE(sl->type) == 0) {
								d = item->max - key;
							}else {
...
not_found:
					...
					value_type update_item(skiplist_t *sl, node_t *item, value_type expectation, value_type new_val) {
						...
						if(!leaf->isfull()) {
							...
							key_type max = 0;
							leaf_t *new_leaf = split_date_node(&max, &min, (item->max - item->min) /2 , leaf);
					...
					value_type sl_insert(skiplist_t *sl, key_type key, value_type expectation, value_type new_val,leaf_t *leaf = NULL) {
						...
						if(nexts[0]) {
							...
							if(!leaf->isfull()) { // if the leaf node is not full
								...
								leaf_t *new_leaf = split_date_node(&max, &min, target_key, leaf);
#ifdef DEBUG
...
#endif
									new_item = node_alloc(n, max, new_val); //todo: remove val in index node
#ifdef DEBUG
